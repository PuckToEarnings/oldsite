...
function hoot_theme_add_welcome_notice() {
	...
	<div id="hoot-welcome-msg" class="notice notice-success is-dismissible">
		...
		<p><?php
			/* Translators: 1 is the link start markup, 2 is link markup end */
			printf( esc_html__( '%1$sGet started with Dispatch%2$s', 'dispatch' ), '<a class="button-secondary" href="' . esc_url( admin_url( 'themes.php?page=dispatch-welcome' ) ) . '">', '</a>' );
			?></p>
	</div>
	<?php
}
...
function hoot_theme_dismiss_welcome_notice() {
	...
	update_option( 'dispatch-dismiss-welcome', 1 ); // echo 'dismiss test';
	wp_die();
}
